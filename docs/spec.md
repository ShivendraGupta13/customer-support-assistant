...
## Architecture Decision: the app *is* `AdkWebServer`
...
- `mvn compile exec:java -Dexec.mainClass=com.google.adk.web.AdkWebServer` starts the app and lists all `demo-*` agents in the Web UI.
...
**Plan:** a small `AppServices` registry populated once at startup (Spring Boot AutoConfiguration in our package is the likely way to still get JPA `DataSource`, Qdrant client, OTel). Agent classes build `ROOT_AGENT` by reading that registry (lazy, so class-load order cannot outrun bean creation). If AutoConfiguration is not picked up by `AdkWebServer`, fall back to our own `main` that starts Spring and then `AdkWebServer.start(agents...)`. Architecture spike verifies which of those two boot paths works on 1.9.x — both keep agents out of constructor injection.
